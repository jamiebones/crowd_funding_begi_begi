...
describe("CrowdFundingFactory", () => {
	const setupFixture = deployments.createFixture(async () => {
		...
		const txn = await factoryContract
			.connect(accounts[0])
			.createNewCrowdFundingContract(fundingDetailsId, amount, duration, category, { value: amountToDeposit })

		const receipt = await txn.wait()

		const cloneAddress = receipt?.logs[1].args[2]

		const instanceOne = await ethers.getContractAt("CrowdFundingContractForBegiBegi", cloneAddress, accounts[0])
	...
	it("Should Deploy a Crowd Funding Contract", async () => {
		...
		let instanceOne = await ethers.getContractAt("CrowdFundingContractForBegiBegi", cloneAddress, accounts[0])
	...
	describe("Donations", function () {
		it("Should accept donations", async function () {
			...
			await expect(() =>
				instanceOne.connect(accounts[1]).giveDonationToCause({ value: ethers.parseEther("1") })
			).to.changeEtherBalances(
				[await accounts[1].getAddress(), instanceOneAddresss],
	...
	describe("Milestone Creation", function () {
		it("Should allow only campaign owner to create a milestone", async function () {
			...
			await expect(
				instanceOne.connect(accounts[1]).creatNewMilestone("milestoneCID")
		...
		it("Should reject milestone creation if a pending milestone exists", async function () {
			...
			await expect(
				instanceOne.connect(accounts[0]).creatNewMilestone("milestoneCID2")
	...
	describe("Voting on Milestone", function () {
		...
		it("Should revert if non-donor tries to vote", async function () {
			const { accounts, instanceOne } = await setupFixture()
			await instanceOne.connect(accounts[0]).creatNewMilestone("milestoneCID")
			await expect(instanceOne.connect(accounts[2]).voteOnMilestone(true)).to.be.revertedWithCustomError(
				instanceOne,
				"YouDidNotDonateToThisCampaign"
			)
		})
		...
		it("Should revert if donor tries to vote twice", async function () {
			...
			await expect(instanceOne.connect(accounts[1]).voteOnMilestone(true)).to.be.revertedWithCustomError(
				instanceOne,
				"YouHaveVotedForThisMilestoneAlready"
			)
		})
	})
	...
	describe("Donor Refund", function () {
		it("Should allow donor to withdraw donation under certain conditions", async function () {
			...
			await expect(() => instanceOne.connect(accounts[1]).retrieveDonatedAmount()).to.changeEtherBalances(
				[await accounts[1].getAddress(), instanceOneAddresss],
				[ethers.parseEther("1.6"), ethers.parseEther("-2.0")]
			)
		})

		it("Should revert if donor has no donation", async function () {
			const { accounts, instanceOne } = await setupFixture()
			await expect(instanceOne.connect(accounts[0]).retrieveDonatedAmount()).to.be.revertedWithCustomError(
				instanceOne,
				"YouDidNotDonateToThisCampaign"
			)
		})
